...
public class AddRemoveMoccmlLanguageProjectNatureHandler extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		if (selection instanceof IStructuredSelection) {
			for (Iterator<?> it = ((IStructuredSelection) selection).iterator(); it
				...
				if (project != null) {
					try {
						IProjectUtils.toggleNature(project, MoccmlLanguageProjectNature.NATURE_ID);
					} catch (CoreException e) {
						Activator.error("Failed to toggle nature "+MoccmlLanguageProjectNature.NATURE_ID+" on "+project.getName(), e);
					}
				}
			}
		}
