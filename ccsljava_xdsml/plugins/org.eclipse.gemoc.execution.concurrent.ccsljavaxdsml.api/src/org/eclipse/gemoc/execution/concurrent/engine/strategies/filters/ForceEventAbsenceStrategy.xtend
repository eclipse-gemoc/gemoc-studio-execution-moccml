 ...
 */
class ForceEventAbsenceStrategy  implements EnumeratingFilteringStrategy {
	 ...
	 */
	@Accessors
	var List<? extends EClass> toBeAbsentTypes

	new(List<? extends EClass> toBePresentTypes) {
		this.toBeAbsentTypes = toBePresentTypes
	}
	...
	private def boolean isToBePresentType(EClass type) {
		toBeAbsentTypes.exists[nit|(nit.name == type.name) || nit.isSuperTypeOf(type)]
	}
}
